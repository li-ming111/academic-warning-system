...
public class CounselorController {
    ...
    public CounselorController(CounselorService counselorService, WarningService warningService,
                               ...
                               ClassManagementRequestService classManagementRequestService, JdbcTemplate jdbcTemplate) {
     ...
     */
    @GetMapping("/students")
    public ApiResponse<List<StudentProfile>> getStudents(@RequestParam(required = false, name = "counselor_id") Long counselorId) {
        try {
     ...
     */
    @GetMapping("/warnings")
    public ApiResponse<List<AcademicWarning>> getWarnings(@RequestParam(required = false, name = "counselor_id") Long counselorId,
    ...
    public ApiResponse<List<Enrollment>> getEnrollments(@RequestParam(required = false, name = "counselor_id") Long counselorId) {
        try {
    ...
    public ApiResponse<List<Map<String, Object>>> getClassActivities(@RequestParam Long counselorId) {
        try {
            // 从数据库查询沟通日志作为班级活动数据
            String sql = "SELECT cl.id as id, '沟通记录' as type, cl.content as title, DATE(cl.created_at) as date " +
                        "FROM communication_logs cl " +
                        "WHERE cl.teacher_id = ? " +
                        "ORDER BY cl.created_at DESC";
            List<Map<String, Object>> activities = jdbcTemplate.queryForList(sql, counselorId);
