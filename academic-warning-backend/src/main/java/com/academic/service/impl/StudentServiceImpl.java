...
public class StudentServiceImpl extends ServiceImpl<StudentProfileMapper, StudentProfile> implements StudentService {
    ...
    private final StudentServiceFeignClient studentServiceFeignClient;
    ...
    public StudentServiceImpl(UserService userService, PasswordEncoder passwordEncoder,
        ...
        this.studentServiceFeignClient = studentServiceFeignClient;
    }
    ...
    public StudentDashboardResponse getStudentDashboard(Long studentId) {
        try {
            // 尝试调用远程学生服务获取仪表盘数据
            log.info("尝试调用远程学生服务获取仪表盘数据: studentId={}", studentId);
            return studentServiceFeignClient.getStudentDashboard(studentId);
        } catch (Exception e) {
            log.warn("远程学生服务调用失败，使用本地数据: {}", e.getMessage());
            // 本地 fallback 逻辑
            StudentDashboardResponse response = new StudentDashboardResponse();
            response.setGpa(getStudentGPA(studentId));
            // 其他本地数据处理
            return response;
        }
    }
