...
import com.academic.feign.UserServiceFeignClient;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService {
    ...
    public LoginResponse login(LoginRequest request) {
        ...
        if (user != null && verifyPassword(request.getPassword(), user.getPassword())) {
            ...
            // 本地验证失败，尝试调用远程用户服务
            try {
                log.info("本地验证失败，尝试调用远程用户服务进行登录");
                response = userServiceFeignClient.login(request);
                log.info("远程登录成功: {}", response.getUsername());
            } catch (Exception e) {
                log.warn("远程用户服务调用失败: {}", e.getMessage());
                if (user == null) {
