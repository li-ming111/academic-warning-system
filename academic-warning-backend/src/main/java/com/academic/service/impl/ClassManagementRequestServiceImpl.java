...
public class ClassManagementRequestServiceImpl extends ServiceImpl<ClassManagementRequestMapper, ClassManagementRequest> 
    ...
    public List<Map<String, Object>> searchClasses(String keyword) {
        ...
        if (!classIds.isEmpty()) {
            // 使用SQL批量查询学生数量
            String sql = "SELECT class_id, COUNT(*) as student_count FROM student_profile WHERE class_id IN (" + 
            ...
            try {
                ...
                for (Map<String, Object> result : results) {
                    ...
                    Long count = ((Number) result.get("student_count")).longValue();
                    studentCountMap.put(classId, count);
                }
            } catch (Exception e) {
                log.error("批量查询学生数量失败", e);
            }
        }

        // 批量查询已批准的管理申请
        Map<Long, Boolean> hasTeacherMap = new HashMap<>();
        Map<Long, Boolean> hasCounselorMap = new HashMap<>();
        if (!classIds.isEmpty()) {
            QueryWrapper<ClassManagementRequest> requestQuery = new QueryWrapper<>();
            requestQuery.in("class_id", classIds).eq("status", "approved");
            List<ClassManagementRequest> requests = requestMapper.selectList(requestQuery);
            
            for (ClassManagementRequest request : requests) {
                if ("teacher".equals(request.getUserType())) {
                    hasTeacherMap.put(request.getClassId(), true);
                } else if ("counselor".equals(request.getUserType())) {
                    hasCounselorMap.put(request.getClassId(), true);
                }
            }
        }
        ...
        for (Class clazz : classes) {
            ...
            // 获取班级学生数
            Long studentCount = studentCountMap.getOrDefault(clazz.getId(), 0L);
            ...
            // 检查是否已有教师或辅导员
            boolean hasTeacher = hasTeacherMap.containsKey(clazz.getId()) || clazz.getTeacherId() != null;
            boolean hasCounselor = hasCounselorMap.containsKey(clazz.getId()) || clazz.getCounselorId() != null;
            map.put("hasTeacher", hasTeacher);
            map.put("hasCounselor", hasCounselor);
