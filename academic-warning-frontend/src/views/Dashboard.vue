...
const loadDashboard = async () => {
  try {
    ...
    console.log('Dashboard 加载 - userId:', userId, 'studentId:', studentId, 'role:', role, 'localStorage 内容:', Object.fromEntries(Object.entries(localStorage)))
    ...
    if (role !== '1' && role !== 'student') {
      console.error('当前用户不是学生角色，无法访问学生Dashboard')
      ElMessage.error('您没有权限访问学生Dashboard，请检查您的角色')
      return
    }
    ...
    if (error.message === '学生不存在') {
      ElMessage.error('您的学生信息不存在，请先注册学生账号')
      // 跳转到注册页面
      setTimeout(() => {
      ...
      // 其他错误使用默认值
      console.warn('其他错误，使用默认值:', error.message)
    }
  }
}
