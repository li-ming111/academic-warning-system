-- 为学号 2023020616 的学生创建账户并插入历年成绩数据

-- 第1步：查询或创建用户
SET @student_id = '2023020616';
SET @password = 'password123'; -- 密码为默认值

-- 先检查用户是否存在
SELECT @user_id := id FROM users WHERE username = @student_id LIMIT 1;

-- 如果不存在则创建用户
INSERT INTO users (username, password, role, enabled, created_at, updated_at)
SELECT @student_id, @password, 1, 1, NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM users WHERE username = @student_id);

-- 重新获取用户ID
SELECT @user_id := id FROM users WHERE username = @student_id LIMIT 1;

-- 第2步：创建或更新学生资料
INSERT INTO student_profile (user_id, student_id, name, college_id, major_id, class_id, enrollment_year, created_at, updated_at)
SELECT @user_id, @student_id, '学生_2023020616', 1, 1, 1, 2023, NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM student_profile WHERE student_id = @student_id)
ON DUPLICATE KEY UPDATE updated_at = NOW();

-- 第3步：获取学生信息中的student_id
SELECT @student_profile_id := id FROM student_profile WHERE student_id = @student_id LIMIT 1;
...
-- 输出结果
SELECT 'Insert complete. User ID:' as result, @user_id as value
UNION ALL
SELECT 'Student ID:', @student_profile_id
UNION ALL
SELECT 'Total scores inserted:', COUNT(*) FROM scores WHERE student_id = @student_profile_id;
